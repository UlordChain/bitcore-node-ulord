...
Node.prototype._setNetwork = function(config) {
  if (config.network === 'test') {
    this.network = Networks.get('test');
...
Node.prototype._startService = function(serviceInfo, callback) {
  ...
  log.info('Starting ' + serviceInfo.name); // TODO: update service name in bitcore-node-ulord.json
  ...
  async.eachSeries(
    ...
    function(service, next) {
      if (self.services[service.name]) {
        log.info('Stopping ' + service.name); // TODO: update service name in bitcore-node-ulord.json
        self.services[service.name].stop(next);
      } else {
        log.info('Stopping ' + service.name + ' (not started)'); // TODO: update service name in bitcore-node-ulord.json
        setImmediate(next);
      }
    },
    callback
