...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/bitcore-node-ulord.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update bitcore-node-ulord.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'bitcore-lib-ulord': '^v0.1.0',
          'bitcore-node-ulord': '^v0.1.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/bitcore-node-ulord.json');
